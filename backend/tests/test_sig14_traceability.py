"""Regression coverage for SIG-14 demo seeding and intake traceability behavior."""
...
class Sig14TraceabilityTests(unittest.TestCase):
    """Verifies the SIG-14 fixture preserves Linear-style intake status for reviewers."""

    def test_sig14_run_detail_preserves_in_progress_traceability(self) -> None:
        """Confirms the seeded SIG-14 run exposes a traceability snapshot pinned to In Progress."""

        # Load the enriched task detail payload the frontend uses on the run room page.
        detail = state.get_run_detail("sig-14", get_settings(), {})

        # Anchor the demo record to the Linear-style ticket id used in product walkthroughs.
        self.assertEqual(detail["ticket"], "SIG-14")
    ...
    def test_fallback_issues_surface_sig14_first_with_intake_status(self) -> None:
        """Confirms intake lists SIG-14 first and keeps tracker status from the frozen snapshot."""
        ...
        # Keep the extremely interesting ticket above the fold for demo operators.
        self.assertEqual(issues[0]["ticket"], "SIG-14")
