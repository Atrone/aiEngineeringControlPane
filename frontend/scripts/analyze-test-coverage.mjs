...
function loadTestSource() {
  ...
  return testFiles.map((filePath) => fs.readFileSync(filePath, 'utf8')).join('\n');
}

function getFunctionName(qualifiedEntry) {
...
function hasTestReference(qualifiedEntry, testSource) {
...
const missing = allFunctions.filter((entry) => !hasTestReference(entry, testSource));
